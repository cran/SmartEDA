...
##' @param gpby default value is True. This parameter is for prepare multidimensional summary, if value is True then summary statistics will be based on multidimensional break else each category variable have seperate summary statistics.
...
##'y = c(1,3,4,0,0,8,0)
##'
##'The complete functionality of this function is explained in vignette help file.
##'@examples
...
ExpCustomStat = function(data,Cvar=NULL,Nvar=NULL,stat=NULL,gpby=TRUE,filt=NULL) {
  options(warn = -1)
  '%ni%' <- Negate('%in%')
  `:=` <- NULL
  `.` <- NULL
